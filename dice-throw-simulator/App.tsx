  ...
  return (
    <div className="w-full h-full relative bg-zinc-900 select-none overflow-hidden touch-none">
      ...
      <div className={`
        ...
        ${settledCount > 0 ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}
      `}>
         <div className="bg-zinc-900/80 backdrop-blur-md border border-amber-500/30 px-8 py-3 rounded-2xl shadow-2xl flex flex-col items-center">
            <span className="text-zinc-400 text-xs uppercase tracking-widest mb-1">Total Score</span>
            <div className="text-5xl font-black text-white flex items-center gap-2">
                <span className={allSettled ? "text-amber-400 animate-pulse-once" : "text-white"}>
                    {totalScore}
                </span>
            </div>
            {allSettled && (
                <div className="flex gap-1 mt-2">
                    {results.map((r, i) => (
                        <span key={i} className="w-6 h-6 rounded bg-zinc-700 flex items-center justify-center text-xs text-zinc-300">
                            {r}
                        </span>
                    ))}
                </div>
      ...
      {/* Settings Panel */}
      {showSettings && (
        <div className="absolute top-20 right-6 w-64 bg-zinc-800/90 backdrop-blur-xl p-5 rounded-2xl shadow-2xl border border-zinc-700 z-20 text-white animate-fade-in-down">
            <h2 className="text-lg font-semibold mb-4 border-b border-zinc-700 pb-2">Settings</h2>
            
            <div className="mb-4">
                <label className="block text-sm text-zinc-400 mb-2 flex justify-between">
                    <span>Dice Count</span>
                    <span className="font-mono bg-zinc-700 px-2 rounded">{diceCount}</span>
                </label>
                <input 
                    type="range" 
                    min="1" 
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-10 w-full max-w-xs px-4">
        <button
          ...
          `}
        >
          <MousePointerClick className={`w-6 h-6 ${isShaking ? 'animate-bounce' : ''}`} />
